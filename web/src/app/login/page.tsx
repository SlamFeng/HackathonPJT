...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await login(email, password, t.auth.loginFailed);
      await refresh();
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-neutral-50 p-6">
      <form onSubmit={onSubmit} className="w-full max-w-sm space-y-4 rounded-2xl bg-white p-8 shadow">
        <h1 className="text-xl font-semibold text-neutral-900">{t.auth.loginTitle}</h1>
        ...
        <div className="space-y-1">
          <label className="text-sm text-neutral-600">{t.auth.password}</label>
          <input
            type="password"
            required
            value={password}
