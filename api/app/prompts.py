...
def vton_tryon_demo_prompt(*, inputs: dict[str, Any], constraints: dict[str, Any] | None) -> str:
    """垂立调试链路（demo lane）的默认提示词 —— 模特海报级。

    与生产 vton_tryon_prompt 相互独立（改这里不影响生产链路）。
    目标：在死守「服装保真」的前提下，把正面垂立的上身图提升到「时尚海报/lookbook」级别——
    从光影、模特表情、干净背景、清晰度四个维度系统调优。管理员也可在「系统设置」用自定义提示词覆盖。
    """
    garment_category = inputs.get("garmentCategory") or '未指定'
    avatar_w = inputs.get("_avatarWidth")
    avatar_h = inputs.get("_avatarHeight")
    dim_hint = f'输出图片必须精确保持 {avatar_w}x{avatar_h} 像素' if (avatar_w and avatar_h) else '输出图片必须与图片 A 的画幅宽高比完全一致'
    return "\n".join([
        '你是顶级时尚电商的资深修图与出图师，目标是产出「时尚海报 / 品牌 lookbook」级别、可直接上架的商品上身图。',
        '',
        '输入图片：',
        '- 图片 A：正面站立（垂立姿态）的目标模特全身照，作为「这个人是谁」与画幅的基准。',
        '- 图片 B：干净的服装单品图，是最终穿着服装的唯一真值（ground truth）。',
        '',
        '核心任务：把图片 B 的服装自然、贴合地穿到图片 A 模特身上，并把整张图提升到时尚海报级质感。',
        '',
        '【第一优先级 · 服装保真（任何美化都不得违反）】',
        '1) 图案/印花/文字/Logo：与图片 B 像素级一致，位置、大小、方向、数量不得增删或挪动；严禁简化、模糊或臆造花纹。',
        '2) 颜色：与图片 B 完全一致（色相/明度/饱和度），不得偏色、褪色或被滤镜带跑；服装本身的颜色不参与下面的"色彩调性"美化。',
        '3) 材质与纹理：还原面料质感（针织/牛仔/丝绸/皮革等）、织纹、车线、光泽与褶皱走向。',
        '4) 版型与结构：领型、领口深浅、袖长袖型、衣长、门襟、纽扣、拉链、口袋、腰带等结构与图片 B 一致；外套/风衣/大衣需显示完整轮廓、翻领、门襟与真实下摆长度。',
        '',
        '【人物身份 · 必须是同一个人】',
        '5) 保持图片 A 人物的身份：脸部五官、发型发色、肤色、体型与垂立站姿、手脚位置、下装与鞋子保持一致；不得换脸、换人或明显改变长相。',
        '6) 服装随人体自然贴合，产生合理褶皱与垂坠，不悬浮、不错位、不穿到别人身上。',
        '',
        '【海报级调优 · 在保真与身份不变的前提下做以下提升】',
        '7) 光影：采用专业棚拍柔光——柔和主光 + 补光压暗部、轻微轮廓光把人物与背景分离；明暗过渡自然、有立体感；曝光均衡，高光不过曝、暗部不死黑；避免平光、避免硬阴影与杂乱投影。',
        '8) 模特表情与气场：自然、放松、自信的时尚模特表情，眼神有神且看向镜头，下巴微收，肩颈放松；不僵硬、不尴尬、不夸张；整体有高级时装大片的气场。',
        '9) 干净背景：替换为无缝棚拍背景，浅中性色（柔白 / 浅灰，可带极轻的渐变或柔和暗角聚焦人物）；背景干净无杂物、无道具、无文字。',
        '10) 清晰度与质感：高分辨率、对焦锐利（脸部与服装均清晰）；保留面料织纹与车线等细节、保留真实皮肤质感（有毛孔与微光泽，不要塑料感/过度磨皮）；边缘干净、无模糊、无噪点、无 JPEG 压缩块或色带。',
        '11) 色彩与影调：整体影调干净通透、白平衡准确、肤色健康自然（服装颜色仍以图片 B 为准，不被整体调色带偏）。',
        '',
        '【构图与画幅】',
        f'12) 人物居中、从头顶到鞋子完整可见、不裁切；{dim_hint}。',
        '',
        f'（前端品类参考：{garment_category}，但一切以图片 B 的实际服装为准。）',
        '',
        '【禁止】不得添加文字/水印/Logo/边框；不得出现多余人物或多余肢体、畸形；不得卡通/插画/CGI 塑料感；不得为了好看而改变服装的图案、颜色或版型。',
        '',
        '失败条件：若服装花纹/颜色/版型与图片 B 不符、或人物身份被改变、或出现裁切/畸形，则判定失败必须重做。',
        '',
        'Return exactly one photorealistic, fashion-poster-grade image: studio soft lighting, clean seamless neutral backdrop, tack-sharp detail, natural confident model expression. The garment must match image B pixel-perfectly in pattern, text, color, material and cut.',
    ])
