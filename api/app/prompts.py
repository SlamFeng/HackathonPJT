...
def body_analysis_prompt() -> str:
    return (
        "你是一个专业的体型分析助手。请分析图片中人物的体型特征，返回严格的JSON格式（不要markdown代码块）。\n"
        "必须返回如下JSON字段：\n"
        "{\n"
        '  "height_estimate": "偏高/中等/偏矮",\n'
        '  "body_shape": "梨形/苹果形/沙漏形/直筒形",\n'
        '  "shoulder_width": "宽/中/窄",\n'
        '  "waist_definition": "明显/一般/不明显",\n'
        '  "style_suggestion": "一句话穿搭建议（20字内）"\n'
        "}\n"
        "注意：只返回JSON，不要其他文字。"
    )


def generate_script_prompt(*, body_analysis_block: str, customer_note: str = "") -> str:
    prompt = (
        "你是一名资深服装导购，请根据以下顾客体型信息，生成一段个性化推荐话术。\n"
        "话术要求：专业、亲切、有说服力，约80-120字。\n\n"
        "顾客体型信息：\n"
        f"{body_analysis_block}\n"
    )
    if customer_note:
        prompt += f"顾客备注：{customer_note}\n"
    prompt += (
        "\n请生成话术（直接输出正文，不要加标题）："
    )
    return prompt
