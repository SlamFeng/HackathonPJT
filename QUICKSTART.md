...
把那两个黑色窗口关掉即可。下次再用，再双击 `start.bat`。
...
## 方式二：Docker 一键启动（适合想要「生产级一致环境」的人）
...
> 两种方式的区别：方式一用本地 SQLite 文件当数据库，零配置、最轻量；Docker 方式用 Postgres，更接近线上生产环境。功能完全一致。
